...
description: List recent local Claude Code or Codex sessions for the current project folder, ask the user to choose one, then share it to ThreadCast.
---
...
4. Call `threadcast.list_recent_sessions` with `limit: 10` and a `source` filter only when the user named Claude Code or Codex. Without `projectPath`, it lists sessions for the current project folder.
