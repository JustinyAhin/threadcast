---
description: Pick a recent Claude Code session from the current project folder and share it to ThreadCast.
---

Help the user share a recent Claude Code session from the current project folder with ThreadCast.
...
4. Call `threadcast.list_recent_sessions` with `{ "limit": 10, "source": "claude-code" }`. Without `projectPath`, it lists sessions for the current project folder.
