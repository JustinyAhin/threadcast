...
3. If `threadcast.login` returns a pending login, show the verification URL and user code and stop.
4. Call `threadcast.list_recent_sessions` with `{ "limit": 10 }`.
5. If the user has not identified a specific session yet, show the list and ask which session ID to share.
6. Once the user chooses a session, call `threadcast.share_session` with `{ "sessionId": "<chosen-session-id>" }`.
